...
1) @MKWMKJYUTVBS (createDeepQNetwork):

Figure out an actual network of layers for poker.  We should probably begin
with a couple of fully connected dense layers consisting of all of the relevant
variables we can think of (i.e. the variables that a poker game analytic engine
would use to make its decisions). The tfjs_examples code we adapted from
(snake-dqn/dqn.js) used conv2d layers because the 9x9 grid in which the snake
moves is just really an image (albeit with very low resolution). We tried to
adapt that code with conv1d layers, but that is stupid because the poker
situation has nothing to do with image "recognition," but rather with the
purely logical system of a general situation that we call a "game." In this
case, using a stack of dense layers appears to be right choice for the network.

2) Make these changes:
	@EUIKMJHYURPO (enable the epsilon-greedy algorithm)
	@MKDLOPIYTGH (create the online and the target networks)
	@EMJKYSDBNHG (do agent.replayBufferSize iterations and remove the return statement)

»*/

//Program Notes«
/*«
(NOT A) Game logic error (CANNOT REPLICATE THIS ERROR!):
Given: Our hero is in the small blind.

If everyone after the blinds folds in the pre-flop round, the game is over
rather than letting our hero make a choice to call/fold/raise.

This "error" was apparent (and not real) because we only do NUM_ITERS 
in agent.playStep, which does not care about finishing the hands. So
what actually happened was that the loop came to an end in the middle
of the final hand (which could have gone all the way to the showdown).

The easiest solution was just to remove the final hand_div @WLMNYFLPO
»*/
/*«

@KDMJRLSMJTPS: ACCOUNTING ERROR!!!
NOT Fixed: @EMNQPLKIUMD by passing the bet amount into play_rest_of_hand()

ACTUALLY FIXED: @SDJUYTRGHJH????

NOT WORKING AFTER CALLING ANOTHER PERSON'S ALL-IN!!!! IT SHOWS
-0 RATHER THAN -36.

The all-in was on the turn, but when the river was dealt at: deal_cards,
the last_bet was set to null. And it seems that -null === -0;

So if the player is all-in, we should NOT set last_bet to null?
@WMMKHBVTYKSSL

So we need to verify that a pre-river all-in keeps...

If the difference between current chips and starting chips is > 0,
return that difference.
Otherwise, just return the negative of the last_bet?
...
//log(`Hand: ${hand_num} (${ROUNDS[round_num]}): ${reward}`);
    this.replayMemory.append([state, action, reward, done, nextState]);
...
//WLMNYFLPO
//The last hand div is always confusing because it usually cuts off in the middle
//of the action
pre.lastChild._del();
//vvvvv
return;//<<<----------				REMOVE THIS!!!!
...
//SDJUYTRGHJH
	let diff = hero.chips - hero.starting_chips;

//Need to make sure that last_bet is NOT set to null on later betting rounds
//in case the player goes all-in before the river...

	if (diff < 0) return -hero.last_bet;
	return diff;
...
//cwarn("SHOWDOWN", hand_num, reward);
	 new_hand();
...
//WMMKHBVTYKSSL
	if (!player.all_in) player.last_bet = null;
...
const play_rest_of_hand = (bet_amt) =>{//«
...
while (!(rv && rv.done === true)){
	iter++;
	if (iter > 10000){
		throw new Error(`Infinite loop ${iter} > 10000`);
		return;
	}
	rv =  seek_to_hero();
}
//cwarn("PLAYREST!!!!", hand_num);
//log(rv);
//EMNQPLKIUMD
//if (rv.reward < 0) rv.reward = -bet_amt;
return rv;

};//»

const game_step = (action) => {//«
...
/*KDMJRLSMJTPS
This is returning the entire stack in case of losing, rather than cur_bet!
*/
		return play_rest_of_hand(cur_bet);
	}
...
//return;
let node = await ("/home/me/Desktop/POKERRANKS.json".toNode());
if (!node) return;
